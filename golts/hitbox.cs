    ...
    public class ObjectHitbox
    {
        ...
        public ObjectHitbox(string path)
        {
            ...
            using(StringReader sr=new StringReader(path))
                lst = Array.ConvertAll<string, int>(sr.ReadToEnd().Split(' '), int.Parse);
        ...
        public bool CollidesWith(ObjectHitbox hitbox, double x1, double y1, double x2, double y2)
        {
            for (int i = 0; i < HitboxPoints.Count; i++)
            {
                ...
                Tuple<double, double> minmax1 = MinMaxPos(Game1.GetDirection(v1, v2) + (float)(Math.PI * 1.5), this, x1, y1);
                Tuple<double, double> minmax2 = MinMaxPos(Game1.GetDirection(v1, v2) + (float)(Math.PI * 1.5), hitbox, x2, y2);
        ...
        {
            double maxdist = -1e7d, mindist = 1e7d;
